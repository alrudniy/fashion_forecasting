...
#
# Changes from v7:
# - Reads data from datasets_with_GT subfolder
# - Extracts keyword from filename to identify GT_keyword column
# - Adds Pass 3: Multivariate with Google Trends
# - Adds Pass 4: Multivariate with Twitter + Google Trends
...
processAllFiles <- function(fname) {
  ...
  write(paste0("Starting ", fname), file = "./results/R_logging_uni_multi_v8.txt", append = TRUE, sep = "\t")
  
  input <- gsub("./datasets_with_GT/", "", fname)
  input <- gsub(".csv", "", input)
  outfile <- "./results/results_all_R_uni_multi_v8.txt"
  ...
  write(paste0("  Keyword: ", keyword, ", GT column: ", gt_col_name), 
        file = "./results/R_logging_uni_multi_v8.txt", append = TRUE, sep = "\t")
