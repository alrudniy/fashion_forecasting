...
#
# Changes from v4:
# - Fix: Multivariate MLP passes full xreg (train+test) to mlp() and no xreg to forecast()
#   mlp() needs xreg length >= length(y) + h during fitting
...
processAllFiles <- function(fname) {
  ...
  write(paste0("Starting ", fname), file = "./results/R_logging_uni_multi_v5.txt", append = TRUE, sep = "\t")
  
  input <- gsub("./datasets/", "", fname)
  input <- gsub(".csv", "", input)
  outfile <- "./results/results_all_R_uni_multi_v5.txt"
  ...
  # FIX: Pass full xreg (train+test) to mlp(), don't pass xreg to forecast()
  # mlp() needs xreg length >= length(y) + h during fitting
  # --------------------------------------------------------------------------
  for (lag in 1:5) {
    tryCatch({
      # Pass full xreg during fitting, forecast() uses it automatically
      mod_mlp <- mlp(amazon.train, lags = 1:lag, xreg = xreg_full, hd = 2, reps = 5)
      pred_mlp <- forecast(mod_mlp, h = hVal)
      acc <- calc_accuracy(as.numeric(pred_mlp$mean), as.numeric(amazon.test))
