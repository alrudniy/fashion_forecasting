# Time Series Analysis - Univariate and Multivariate Models (V6)
...
#
# Changes from v5:
# - Fix: Multivariate MLP uses xreg.lags=0 to disable lagged xreg features
#   This prevents subscript out of bounds errors on shorter series
...
processAllFiles <- function(fname) {
  ...
  write(paste0("Starting ", fname), file = "./results/R_logging_uni_multi_v6.txt", append = TRUE, sep = "\t")
  
  input <- gsub("./datasets/", "", fname)
  input <- gsub(".csv", "", input)
  outfile <- "./results/results_all_R_uni_multi_v6.txt"
  ...
  # Full xreg for MLP (train + test concatenated)
  # mlp() requires xreg length >= length(y) + h during fitting
  ...
  # FIX: Use xreg.lags=0 to disable lagged xreg features (prevents subscript errors)
  # --------------------------------------------------------------------------
  for (lag in 1:5) {
    tryCatch({
      # Pass full xreg during fitting with xreg.lags=0 to use only contemporaneous xreg
