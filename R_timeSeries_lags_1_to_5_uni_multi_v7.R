...
#
# Changes from v6:
# - Fix: Replaced nnfor::mlp() with forecast::nnetar() for multivariate MLP
#   nnetar() is more stable and properly supports xreg
...
processAllFiles <- function(fname) {
  ...
  write(paste0("Starting ", fname), file = "./results/R_logging_uni_multi_v7.txt", append = TRUE, sep = "\t")
  
  input <- gsub("./datasets/", "", fname)
  input <- gsub(".csv", "", input)
  outfile <- "./results/results_all_R_uni_multi_v7.txt"
  ...
  # xreg matrices for nnetar (separate train and test)
  ...
  # Using forecast::nnetar instead of nnfor::mlp
  # --------------------------------------------------------------------------
  for (lag in 1:5) {
    tryCatch({
      # nnetar with specified lags (p = number of lags)
  ...
  # --------------------------------------------------------------------------
  # Multivariate: NNETAR with xreg for lags 1-5
  # Using forecast::nnetar which properly supports xreg
  # --------------------------------------------------------------------------
  for (lag in 1:5) {
    tryCatch({
      # nnetar with xreg (Twitter as external regressor)
